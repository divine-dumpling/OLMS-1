...
<div class="content-wrapper">
    ...
    if (isset($_GET['id'])) {
        ...
        $date = date('d-m-Y');
        $query = mysqli_query($conn, "UPDATE issued_book SET return_date = '$date' where id = '$id';");
        if ($query) {
...
<!--                                <th style="width:10%">Book Image</th>-->
                            ...
                            <tbody>
                                <?php
                                    $query = mysqli_query($conn, "SELECT issued_book.id, issued_book.book_id, issued_book.issued_date, students.fname, students.lname, students.roll, students.phone_no, students.department_id, books.book_name, books.book_image FROM issued_book INNER JOIN students ON students.id = issued_book.student_id INNER JOIN books ON books.id = issued_book.book_id WHERE issued_book.return_date = ''");
                                    $i = 0;
...
<!--                                        <td><img src="../dist/images/book/--><?//= $result['book_image']?><!--" alt="" style="width: 50px; height: 50px;"></td>-->
                                        <td><?= date('M d, Y', strtotime($result['issued_date']))?>
                                        </td>
